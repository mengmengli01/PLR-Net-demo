...
def weights_init(m):
    if isinstance(m, nn.Conv2d):
        ...
        if m.bias is not None:
            m.bias.data.zero_()
    print("init weight is ok")


class LossReducer(object):
    def __init__(self, cfg):
        # loss_joff: 0.25;loss_jloc: 8.0;loss_mask: 1.0;loss_afm : 0.1;loss_remask : 1.0;
...
def parse_args():
    ...
    parser.add_argument("--config-file",
                        ...
                        type=str,
                        default="../config-files/crowdai_resunet101.yaml",
                        )

...
class TestDatasetWithAnnotations(dset.coco.CocoDetection):
    def __init__(self, root, ann_file, transform=None):
        ...
        self.id_to_img_map = {k: v for k, v in
                              enumerate(self.ids)} 
        self._transforms = transform

    def __getitem__(self, idx):
        img, annos = super(TestDatasetWithAnnotations, self).__getitem__(
            idx) 
            ...
            for i, segm in enumerate(segmentations):  
                ...
                if i == 0: 
                    ...
                    if poly.area > 0: 
                        ...
                        convex_index = [(p == convex_point).all(1).any() for p in
                                        points]  
                    ...
                    cv2.drawContours(seg_mask, [np.int0(interior_contour)], -1, color=0,
                                     thickness=-1)  
